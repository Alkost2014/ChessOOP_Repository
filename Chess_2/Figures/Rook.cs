...
namespace Chess_2
{
    class Rook : Figure // Производный класс Ладья
    {
        // Пока лишнее!!!
        //public Rook(string figureImage, FigureColor figureColor, Coords figureCoords) : base(figureImage, figureColor, figureCoords)
        //{

        //}

        public Rook(string figureImage, FigureColor figureColor) : base(figureImage, figureColor)
        {
        ...
        public override void Move(Coords currentCoords, Coords newCoords, bool inputIsFirstPlayer, Board board)
        {
            if (inputIsFirstPlayer)
            {
                ...
                board[newCoords.y - 1, newCoords.x - 1] = new Rook("л", FigureColor.Green);
            }
            else
            {
                board[currentCoords.y - 1, currentCoords.x - 1] = null;
                board[newCoords.y - 1, newCoords.x - 1] = new Rook("л", FigureColor.Red);
            }
        }
    }
}
