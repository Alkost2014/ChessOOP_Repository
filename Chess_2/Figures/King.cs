...
namespace Chess_2
{
    class King : Figure // Производный класс Король
    {
        // Пока лишнее!!!
        //public King(string figureImage, FigureColor figureColor, Coords figureCoords) : base(figureImage, figureColor, figureCoords)
        //{

        //}

        public King(string figureImage, FigureColor figureColor) : base(figureImage, figureColor)
        {
        ...
        public override bool IsValidEndPosition(bool inputIsFirstPlayer, Coords currentCoords, Coords newCoords, Board board)
        {
            ...
            if ((newCoords.x != currentCoords.x) && (newCoords.y != currentCoords.y) || 
                (Math.Abs(newCoords.x - currentCoords.x) > 1) || (Math.Abs(newCoords.y - currentCoords.y) > 1) || 
        ...
        public override void Move(Coords currentCoords, Coords newCoords, bool inputIsFirstPlayer, Board board)
        {
            if (inputIsFirstPlayer)
            {
                ...
                board[newCoords.y - 1, newCoords.x - 1] = new King("К", FigureColor.Green);
            }
            else
            {
                board[currentCoords.y - 1, currentCoords.x - 1] = null;
                board[newCoords.y - 1, newCoords.x - 1] = new King("К", FigureColor.Red);
            }
        }
    }
}
